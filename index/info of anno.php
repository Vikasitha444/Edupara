...
if(isset($_COOKIE['how_classes_do']) || isset($_COOKIE['searched_query']) || isset($_COOKIE['selected_category']) || isset($_COOKIE['selected_category_by_user']) || isset($_COOKIE['students_grade'])){
    setcookie("how_classes_do","",-1,'/');
    setcookie("students_grade","",-1,'/');
}
...
if(isset($_GET['Search_bar']) && isset($_GET['selected_category'])){
    ...
    setcookie("searched_query",$searched_query,time()+300,'/');
    setcookie("selected_category",$selected_category,time()+300,'/');
    
}



if(isset($_GET['tmpemail']) && isset($_GET['tmppassword']) && isset($_GET['newcomer']) && isset($_COOKIE['selected_category']) && isset($_COOKIE['searched_query'])){
    

    $email_from_login = $_GET['tmpemail'];
    $password_from_login = $_GET['tmppassword'];
    $new_comer_state = $_GET['newcomer'];
    $selected_category = $_COOKIE['selected_category'];
    $search_query_from_cookie = urlencode($_COOKIE['searched_query']);
    
    
    
    setcookie("EmailFromLogin", $email_from_login, time()+86400,'/','');
    setcookie("PasswordFromLogin",$password_from_login,time()+86400,'/','');
    header("Location: ../main_ads_grid/Mathamatics Worksheets & Teaching Resources _ Teachers Pay Teachers.php?new_comer_state=$new_comer_state&selected_category=$selected_category&Search_bar=$search_query_from_cookie");
    setcookie("searched_query",'',-1,'/');
}
...
<meta charset="utf-8">
    ...
    <title>Grade, district and city of anonymous session | Edupara.lk</title>
...
<body>
            ...
            error_reporting(E_ALL);
                ...
                if(isset($_GET['selected_category']) && isset($_GET['Search_bar'])){
                    ...
                    if($selected_category == 'subject'){
                        $subject_name = $search_query;
                    
                            //Only 6-OL Grades
                            if(
                                $subject_name == "Buddhism" ||
                                $subject_name == "Christianity" ||
                                $subject_name == "Catholic" ||
                                $subject_name == "Science" ||
                                $subject_name == "Geography" ||
                                $subject_name == "Dancing" ||
                                $subject_name == "Western Music" ||
                                $subject_name == "Drama & Theatre"||
                                $subject_name == "Health & Physical Education"||
                                $subject_name == "Shivenary" ||
                                $subject_name == "Sinhala Language" ||
                                $subject_name == "Mathematics" ||
                                $subject_name == "History" ||
                                $subject_name == "Life skills & Citizenshipn Education" ||
                                $subject_name == "Eastern Music" ||
                                $subject_name == "Practical & Technical Studies" ||
                                $subject_name == "Tamil Language"
                            )
                            
                                {
                                    $grade_one_to_nine_subject = true;
                                }
                            
                            //Only OL                        
                            else if(
                                $subject_name == "Catholism" ||
                                $subject_name == "Islam" ||
                                $subject_name == "Sinhala Language & Literature" ||
                                $subject_name == "Citizenship Education" ||
                                $subject_name == "Enterpreneurial Education" ||
                                $subject_name == "Business Studies & Accounts" ||
                                $subject_name == "Traditional Dancing" ||
                                $subject_name == "Sinhala Literature" ||
                                $subject_name == "English Literature" ||
                                $subject_name == "Information & Communication Technology" ||
                                $subject_name == "Agriculture & Food Technology" ||
                            ...
                            //Both OL and AL Subjects will be here
                            else if(
                                $subject_name == 'Information Communication Technology'
                    ...
                    }elseif($selected_category == 'teacher'){
                        $teachers_full_name = $search_query;
                        ...
                        JOIN teachers USING (T_ID);";
                          ...
                          if ($resaltcheck5 > 0) {
                            ...
                            for ($k=0; $k < count($datas5); $k ++) { 
                                ...
                                $teachers_full_name_from_selects = "$teachers_fname_select $teachers_sname_select";
                                
                                if($teachers_full_name == $teachers_full_name_from_selects){
                                    echo "<script> $(function(){teacher_grades('$grades_without_repeting');}) </script>"; //running a function in jq
                                }
                        ...
                        for ($d=1; $d <= $count_of_subject_columns; $d++) { //If you incrise the size of database. Please update this
                            ...
                            for ($c=0; $c < count($datas6); $c++) { 
                                if(($datas6[$c]["INSTITUTE__$d"]) != NULL){
                                    if($datas6[$c]["INSTITUTE__$d"] == $institute){
                                        array_push($all_institute_grades,$datas6[$c]["GRADE__$d"]);
                                    }
                        ...
                        $array_to_a_string = implode("|",$all_institute_grades);
                        
    
                        echo "<script>$(function(){institute_grades('$array_to_a_string');});</script>";
            ...
            <!-- show the selected subject, teacher etc starts here -->
                <script>
                    $(function(){
                        var selected_subject_name = "<?php echo $subject_name?>";
                        $("#selected_subject").attr('value',selected_subject_name);
                        $("label[for=selected_subject]").text('Selected Subject :');
                    });
                </script>
                <script>
                    $(function(){
                        var selected_teacher_name = "<?php echo $teachers_full_name; ?>";
                        $("#selected_subject").attr('value',selected_teacher_name);
                        $("label[for=selected_subject]").text('Selected Teacher :');
                    });
                </script>
                <script>
                    $(function(){
                        var selected_institute_name = "<?php echo $institute; ?>";
                        $("#selected_subject").attr('value',selected_institute_name);
                        $("label[for=selected_subject]").text('Selected Institute :');
                        $("label[for=grade]").text('Grades available in this institute');
                    });
                </script>
            <!-- show the selected subject, teacher etc starts here -->
            
            <!-- Grades variation accoding to grade starts here -->
                    <script>
                        $(document).ready(function(){
                            var grade_one_to_nine_subject = "<?php if($grade_one_to_nine_subject === true){echo true;}else{echo false;}?>";
                            if(grade_one_to_nine_subject == true){
                                $("#grade").find("option[value=AL]").prop('disabled',true);
                                $("#grade").find("option[value=OTHER]").prop('disabled',true);
                    ...
                    <script>
                        $(document).ready(function(){
                            var ol_subject = "<?php if($ol_subject === true){echo true;}else{echo false;}?>";
                            if(ol_subject == true){
                                $("#grade").find("option[value=OL]").prop('selected',true);
                    ...
                    <script>
                        $(document).ready(function(){
                            var both_OL_and_AL = "<?php if($both_OL_and_AL === true){echo true;}else{echo false;}?>";
                            if(both_OL_and_AL == true){
                                $("#grade").find("option[value=AL]").prop('disabled',false);
                                $("#grade").find("option[value=OL]").prop('selected',true);
                                $("#grade").find("option[value=grade_1]").prop('disabled',true);
                                $("#grade").find("option[value=grade_2]").prop('disabled',true);
                                $("#grade").find("option[value=grade_3]").prop('disabled',true);
                                $("#grade").find("option[value=grade_4]").prop('disabled',true);
                                $("#grade").find("option[value=grade_5]").prop('disabled',true);
                                $("#grade").find("option[value=grade_6]").prop('disabled',true);
                                $("#grade").find("option[value=grade_7]").prop('disabled',true);
                                $("#grade").find("option[value=grade_8]").prop('disabled',true);
                                $("#grade").find("option[value=grade_9]").prop('disabled',true);
                    ...
                    <script>
                        $(document).ready(function(){
                            var al_subject = "<?php if($al_subject === true){echo true;}else{echo false;}?>";
                            if(al_subject == true){
                                $("#grade").find("option[value=AL]").prop('selected',true);
                                $("#grade").prop('disabled',true);
                    ...
                    <script>
                        $(document).ready(function(){
                            var all_grades_common = "<?php if($all_grades_common === true){echo true;}else{echo false;}?>";
                            if(all_grades_common == true){
                            }
            ...
            <!-- Grades variation accoding to grade ends here -->            
            

               
            
            <!-- Teachers variation accoding to grade starts here -->
                    <!-- if user selects a teacher to find -->
                    <!-- php sends what are the subjects selected teacher is teaching -->
                    <script>
                        function teacher_grades(teacher_teaching_grades){
                            //Firstly disable all the options
                            $(function(){
                                    $("#grade option").prop('disabled', true);
                                
                                
                                    if(teacher_teaching_grades.includes('grade_9')){
                                        $("#grade").find("option[value=grade_9]").prop('selected',true);
                                        $("#grade").find("option[value=grade_9]").prop('disabled',false);
                                    }

                                    if(teacher_teaching_grades.includes('grade_8')){
                                        $("#grade").find("option[value=grade_8]").prop('selected',true);
                                        $("#grade").find("option[value=grade_8]").prop('disabled',false);
                                    }

                                    if(teacher_teaching_grades.includes('grade_7')){
                                        $("#grade").find("option[value=grade_7]").prop('selected',true);
                                        $("#grade").find("option[value=grade_7]").prop('disabled',false);
                                    }

                                    if(teacher_teaching_grades.includes('grade_6')){
                                        $("#grade").find("option[value=grade_6]").prop('selected',true);
                                        $("#grade").find("option[value=grade_6]").prop('disabled',false);
                                    }

                                    if(teacher_teaching_grades.includes('grade_5')){
                                        $("#grade").find("option[value=grade_5]").prop('selected',true);
                                        $("#grade").find("option[value=grade_5]").prop('disabled',false);
                                    }


                                    if(teacher_teaching_grades.includes('grade_4')){
                                        $("#grade").find("option[value=grade_4]").prop('selected',true);
                                        $("#grade").find("option[value=grade_4]").prop('disabled',false);
                                    }

                                    if(teacher_teaching_grades.includes('grade_3')){
                                        $("#grade").find("option[value=grade_3]").prop('selected',true);
                                        $("#grade").find("option[value=grade_3]").prop('disabled',false);
                                    }

                                    if(teacher_teaching_grades.includes('grade_2')){
                                        $("#grade").find("option[value=grade_2]").prop('selected',true);
                                        $("#grade").find("option[value=grade_2]").prop('disabled',false);
                                    }

                                    if(teacher_teaching_grades.includes('grade_1')){
                                        $("#grade").find("option[value=grade_1]").prop('selected',true);
                                        $("#grade").find("option[value=grade_1]").prop('disabled',false);
                                    }

                                    if(teacher_teaching_grades.includes('OL')){
                                        $("#grade").find("option[value=OL]").prop('selected',true);
                                        $("#grade").find("option[value=OL]").prop('disabled',false);
                                    }
                                    
                                    if(teacher_teaching_grades.includes('AL')){
                                        $("#grade").find("option[value=AL]").prop('selected',true);
                                        $("#grade").find("option[value=AL]").prop('disabled',false);
                                    }
            ...
            <!-- Institute variation accoding to grade starts here -->
                <script>
                    function institute_grades(ins_grades){
                        $(function(){
                            $("#grade option").prop('disabled', true); //Disable everything
                            
                            if(ins_grades.includes('grade_1')){
                                $("#grade").find("option[value=grade_1]").prop('selected',true);
                                $("#grade").find("option[value=grade_1]").prop('disabled',false);
                            }

                            if(ins_grades.includes('grade_2')){
                                $("#grade").find("option[value=grade_2]").prop('selected',true);
                                $("#grade").find("option[value=grade_2]").prop('disabled',false);
                            }

                            if(ins_grades.includes('grade_3')){
                                $("#grade").find("option[value=grade_3]").prop('selected',true);
                                $("#grade").find("option[value=grade_3]").prop('disabled',false);
                            }

                            if(ins_grades.includes('grade_4')){
                                $("#grade").find("option[value=grade_4]").prop('selected',true);
                                $("#grade").find("option[value=grade_4]").prop('disabled',false);
                            }

                            if(ins_grades.includes('grade_5')){
                                $("#grade").find("option[value=grade_5]").prop('selected',true);
                                $("#grade").find("option[value=grade_5]").prop('disabled',false);
                            }

                            if(ins_grades.includes('grade_6')){
                                $("#grade").find("option[value=grade_6]").prop('selected',true);
                                $("#grade").find("option[value=grade_6]").prop('disabled',false);
                            }


                            if(ins_grades.includes('grade_7')){
                                $("#grade").find("option[value=grade_7]").prop('selected',true);
                                $("#grade").find("option[value=grade_7]").prop('disabled',false);
                            }

                            if(ins_grades.includes('grade_8')){
                                $("#grade").find("option[value=grade_8]").prop('selected',true);
                                $("#grade").find("option[value=grade_8]").prop('disabled',false);
                            }

                            if(ins_grades.includes('grade_9')){
                                $("#grade").find("option[value=grade_9]").prop('selected',true);
                                $("#grade").find("option[value=grade_9]").prop('disabled',false);
                            }

                            if(ins_grades.includes('OL')){
                                $("#grade").find("option[value=OL]").prop('selected',true);
                                $("#grade").find("option[value=OL]").prop('disabled',false);
                            }

                            if(ins_grades.includes('AL')){
                                $("#grade").find("option[value=AL]").prop('selected',true);
                                $("#grade").find("option[value=AL]").prop('disabled',false);
                            }
            ...
            <!-- Institute variation accoding to grade starts here -->
                        ...
                        <div class="d-flex align-items-center justify-content-center" style="height: 100%; margin-top: 10%; margin-bottom:10%;">
                            <div class="container d-flex  justify-content-center" >
                                <div class="">
                                    <div class="col-md-6">
                                        <div class="card rounded-0 shadow">
                                            <div class="card-body">
                                                <h3 class="text-center">Please Select Your Grade, District And city</h3><br/><br>
                                                
                                                <form action="info of anno.php?" method="POST">

                                                    <div class="form-group">
                                                        <label for="selected_subject">Selected Subject: </label>
                                                        <input type="text" class="form-control" disabled id="selected_subject">
                                                    </div>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="dropdown">
                                                            <select class="form-control" id="selectSubject" name="district" required>
                                                    ...
                                                    <button type="submit" class="btn btn-primary" name="submit">Search best results for me!</button> 
...
<!-- If user registered -->
    ...
    $sql3="SELECT MAX(S_ID) FROM `students`;";
    ...
    if ($resaltcheck3 > 0) {
      ...
      $next_sid = ($datas3[0]['MAX(S_ID)']) + 1;
    }
    
    
if(isset($_POST['submit'])){
    $full_name = "ANONYMOUS_$next_sid";
    $email = "ANONYMOUS_$next_sid@anonymous.login";
    $grade = $_POST['grade'];
    $district = $_POST['district'];
    $password = "ANONYMOUS_$next_sid LOGIN_SESSION";
    $confirm_password = "ANONYMOUS_$next_sid LOGIN_SESSION";
    $contact_number = rand(1111111111,9999999999);
    $whatsapp_number = rand(1111111111,9999999999);
    ...
    //Read this: I just found out AL is not recoing in the DB. so I made this statement. If this is annoying Just remove this out.
    if($grade == ''){$grade = 'AL';}
    ...
    if($password == $confirm_password){
            ...
            if($resultInsert === TRUE){
                ...
                echo "<script>window.location.replace('info of anno.php?tmpemail=$email&tmppassword=$password&newcomer=true&selected_category=$selected_category&Search_query=$search_query');</script>";
